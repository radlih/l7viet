...
const cacheHeaders = {
  ...
  async function handleRequest(request) {
    ...
    request.headers.set("user-agent", ua ? ua : "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36")
    
    const url = new URL(request.url)
    request.headers.set("X-Forwarded-For", `65.222.202.53`)  /* Send X-Forwarded-For header in case CF checks IP */ 
    
    let response = null
    if (url.pathname === "/narul") {
      /* /path (bypass path) */
      response = handleBypass(request) 
    } else {
      response = await fetch(request)
    }
    response.headers.set(cacheHeaders)
    
    return response
  }
  
  function handleBypass(request) {
    const body = "Bypass successful!"
